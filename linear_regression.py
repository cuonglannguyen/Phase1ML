# -*- coding: utf-8 -*-
"""
Created on Fri Jan 21 21:53:12 2022

@author: Admin
"""
import numpy as np

data = open('dataset_regression.txt','r')
regression_data=data.readlines()
column_number = 16
temp_Y = []
for y in regression_data:
    temp_Y.append(y.split()[column_number])
data.close()
Y = np.array(temp_Y,dtype=float)
#print(Y)
with open('dataset_regression.txt', 'r') as f:
    X = [[num for num in line.split()] for line in f]
for item in X:
    item.pop(0)
    item.pop(15)
X = [[float(x) for x in i] for i in X]
